...
export class ProductComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    //הבאת הרשימה של המשתמשים לבחירת המשתמש שאליו שייך החפץ
      ...
      this.product.UserId = this.ProductService.userOfTheProduct;
  ...
  onCategoryChanged(isMain: boolean) {
    ...
    this.ParameterService.getParametersOfCategory(categoryID).subscribe(
      (res: Parameter[]) => {
        if (res != null) {
  ...
  OnAddProduct(myForm: NgForm) {
    ...
    for (let i = 0; i < this.parametersAreExist.length; i++) {
      this.ParameterOfProductAreExist[i].ParameterId = this.parametersAreExist[
        i
      ].ParameterId;
    }
    ...
    for (var i = 0; i < this.NewParameters.length; i++) {
      this.NewParameters[i].CategoryId =
