...
export class MatchesComponent implements OnInit {
  ...
  findAddress(){
    ...
    this.matches.forEach(e=>{
      if(e.AddressDescription)
      this.s=e.AddressDescription;
      else
     this.s=this.ProductService.getAddressByCoord(e.AddressPointX,e.AddressPointY);
  ...
  async sendEmail(Email: string) {
    const { value: formValues } = await Swal.fire({
      ...
      html:
        ...
        '<input id="body" class="swal2-input">',
    ...
    if (formValues) {
      // this.UserService.sendMail(formValues).subscribe((res: string) => {
      //   if (res) {
      //     Swal.fire({
      //       type: 'success',
